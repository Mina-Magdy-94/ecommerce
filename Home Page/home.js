...
function addToCart(btnId) {
  ...
  // todo: set cookie
  setCookie("cart", btoa(JSON.stringify(data.cart)), 1);
...
function loadCartFromCookie() {
  data.cart = getCookie("cart") ? JSON.parse(atob(getCookie("cart"))) : [];
}
...
function scrollFunction() {
  var sectionOfProducts = document.getElementById("products");
  if (sectionOfProducts.scrollTop > 35) {
...
function topFunction() {
  sectionOfProducts.scrollTop = 0;
}

sectionOfProducts.onscroll = function () {
  scrollFunction();
};
